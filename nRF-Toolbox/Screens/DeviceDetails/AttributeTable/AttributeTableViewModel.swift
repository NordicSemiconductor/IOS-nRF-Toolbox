...
private extension ViewModel {
    func readAttributes() async throws -> [Attribute] {
        ...
        let services = try await peripheral.discoverServices(serviceUUIDs: nil).timeout(1, scheduler: DispatchQueue.main).value
        for s in services {
            at.addService(s)
            
            let characteristics = try await peripheral.discoverCharacteristics(nil, for: s).timeout(1, scheduler: DispatchQueue.main).value
            for c in characteristics {
                
                at.addCharacteristic(c)
                
                let descriptors = try await peripheral.discoverDescriptors(for: c).timeout(1, scheduler: DispatchQueue.main).value
