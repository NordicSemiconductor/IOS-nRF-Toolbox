...
class DFUViewController: BaseViewController, ScannerDelegate, FileSelectionDelegate, LoggerDelegate, DFUServiceDelegate, DFUProgressDelegate, StoryboardInstantiable {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
            if (segue.identifier == "scan") {
                ...
                let appFilecsVC = barViewController?.viewControllers?.first as? AppFilesViewController
                appFilecsVC?.fileDelegate = self
                let userFilesVC = barViewController?.viewControllers?.last as? UserFilesViewController
