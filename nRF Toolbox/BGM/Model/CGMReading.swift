...
struct CGMReading {
    ...
    init(_ bytes : UnsafeMutablePointer<UInt8>) {
        ...
        let currentMeasurementSize = CharacteristicReader.readUInt8Value(ptr: &pointer)
        ...
        self.unit                        = .mgDl
        self.timeOffsetSinceSessionStart = TimeInterval(60 * CharacteristicReader.readSInt16Value(ptr: &pointer))
        ...
        if self.sensorStatusAnnunciationPresent {
            self.sensorStatusAnnunciation = CharacteristicReader.readUInt32Value(ptr: &pointer)
        } else {
        ...
        if self.sensorTrendInfoPresent {
            self.trendInfo = CharacteristicReader.readSFloatValue(ptr: &pointer)
        } else {
            self.trendInfo = nil
        }
        ...
        if self.sensorQualityPresent {
            self.quality = CharacteristicReader.readSFloatValue(ptr: &pointer)
        } else {
            self.quality = nil
        }
...
extension CGMReading: Equatable {
    
    static func == (lhs: CGMReading, rhs: CGMReading) -> Bool {
