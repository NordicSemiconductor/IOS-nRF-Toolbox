...
class ProximityViewController: BaseViewController, CBCentralManagerDelegate, CBPeripheralDelegate, ScannerDelegate, CBPeripheralManagerDelegate, StoryboardInstance {
    ...
    @IBAction func aboutButtonTapped(_ sender: AnyObject) {
        self.showAbout(message: AppUtilities.getHelpTextForService(service: .proximity))
    }
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        proximityImmediateAlertServiceUUID      = CBUUID(string: ServiceIdentifiers.proximityImmediateAlertServiceUUIDString)
        proximityLinkLossServiceUUID            = CBUUID(string: ServiceIdentifiers.proximityLinkLossServiceUUIDString)
        proximityAlertLevelCharacteristicUUID   = CBUUID(string: ServiceIdentifiers.proximityAlertLevelCharacteristicUUIDString)
    ...
    @objc func applicationDidEnterBackgroundCallback() {
        ...
        AppUtilities.showBackgroundNotification(message: "You are still connected to \(name).")
    }
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        ...
        DispatchQueue.main.async {
            ...
            if AppUtilities.isApplicationInactive() {
                AppUtilities.showBackgroundNotification(message: "\(peripheral.name ?? "Device") is within range!")
            }
        }
        ...
        DispatchQueue.main.async {
            ...
            if error != nil {
                ...
                if AppUtilities.isApplicationInactive() {
                    ...
                    AppUtilities.showAlert(title: "PROXIMITY", andMessage: message, from: self)
                }
                self.playSoundOnce()
            } else {
                self.connectionButton.setTitle("CONNECT", for: .normal)
