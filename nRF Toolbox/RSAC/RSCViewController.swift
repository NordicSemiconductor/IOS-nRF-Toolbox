...
class RSCViewController: BaseViewController, CBCentralManagerDelegate, CBPeripheralDelegate, ScannerDelegate, StoryboardInstantiable {
    ...
    @IBAction func aboutButtonTapped(_ sender: AnyObject) {
        showAbout(message: AppUtilities.getHelpTextForService(service: .rsc))
    }
    ...
    required init?(coder aDecoder: NSCoder) {
        stepsNumber                      = 0
        tripDistance                     = 0
        rscServiceUUID                   = CBUUID(string: ServiceIdentifiers.rscServiceUUIDString)
        rscMeasurementCharacteristicUUID = CBUUID(string: ServiceIdentifiers.rscMeasurementCharacteristicUUIDString)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "scan" {
            ...
            let scanController = nc.children.first as! ScannerViewController
            scanController.filterUUID = rscServiceUUID
            scanController.delegate = self
        }
    }
    //MARK: - RSCViewController implementation
    @objc func timerFired(timer aTimer : Timer) {
        ...
        strides.text = String(format:"%d", stepsNumber!)
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        DispatchQueue.main.async {
            ...
            if characteristic.uuid == self.batteryLevelCharacteristicUUID {
                ...
                let speedValue = Float(CharacteristicReader.readUInt16Value(ptr: &array)) / 256.0 * 3.6
                self.speed.text = String(format:"%.1f", speedValue)
                
                self.cadenceValue = CharacteristicReader.readUInt8Value(ptr: &array)
                ...
                if strideLengthPresent {
                    self.stripLength = CharacteristicReader.readUInt8Value(ptr: &array) // value in Centimeters
                }
                
                if totalDistancePresent {
                    let distanceValue = CharacteristicReader.readUInt32Value(ptr: &array) //value in Centimeters
