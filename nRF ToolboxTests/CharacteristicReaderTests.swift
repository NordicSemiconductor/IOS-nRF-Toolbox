...
class CharacteristicReaderTests: XCTestCase {
    ...
    func testCharacteristicReaderAdvancesAfterRead() {
        ...
        var result : UInt8 = 10
        for aValue in array {
            result = CharacteristicReader.readUInt8Value(ptr: &testData)
            XCTAssertEqual(result, aValue, "Expected result does not match output!")
        }
    }
}
